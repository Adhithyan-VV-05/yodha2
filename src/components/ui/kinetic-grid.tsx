...
const CELL_SIZE_DESKTOP = 55;
const CELL_SIZE_MOBILE = 42;
const INFLUENCE_RADIUS_DESKTOP = 260;
const INFLUENCE_RADIUS_MOBILE = 200;
const MAX_WARP = 26;
const DOT_SPACING = 28;
const LERP_SPEED = 0.35; // Fast responsiveness for desktop
...
export default function KineticGrid({
  ...
  const mobileTimeoutRef = useRef<ReturnType<typeof setTimeout> | number | null>(null);
  const desktopIdleTimeoutRef = useRef<ReturnType<typeof setTimeout> | number | null>(null);
  ...
  const draw = useCallback(
    (now: number) => {
      ...
      const influenceRadius = isMobile ? INFLUENCE_RADIUS_MOBILE : INFLUENCE_RADIUS_DESKTOP;
      ...
      for (let i = ripples.length - 1; i >= 0; i--) {
        ...
        // On mobile, fade out ripples faster (2.2x speed) so grid resets quickly to normal
        const fadeSpeed = isMobile ? 2.2 : 1.2;
        const expandSpeed = isMobile ? 480 : 400;
        r.radius = Math.max(0, age * expandSpeed);
        r.opacity = Math.max(0, 1 - age * fadeSpeed);
  ...
  const animate = useCallback(
    (now: number) => {
      ...
      if (t.x === -9999) {
        // Quick smooth return to baseline flat grid on mobile/release
        m.x = lerpN(m.x, -9999, 0.15);
        m.y = lerpN(m.y, -9999, 0.15);
      } else {
  ...
  useEffect(() => {
    ...
    const resetMobileTargetSoon = (delayMs = 250) => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (desktopIdleTimeoutRef.current) clearTimeout(desktopIdleTimeoutRef.current);
      // Gradually return background to normal after cursor stops moving (300ms)
      desktopIdleTimeoutRef.current = setTimeout(() => {
        targetMouseRef.current = { x: -9999, y: -9999 };
      }, 300);
    };
