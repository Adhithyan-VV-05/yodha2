    ...
    <div ref={containerRef} className={`relative w-full select-none ${className}`}>
      ...
      <button
        ...
        className={`w-full bg-[#080b2d]/90 hover:bg-[#0e123d] border ${
          isOpen ? "border-blue-400 ring-2 ring-blue-500/30" : "border-blue-500/40 hover:border-blue-400"
        } rounded-2xl px-4 py-3 text-xs sm:text-sm font-mono font-bold text-white flex items-center justify-between shadow-[0_0_20px_rgba(59,130,246,0.15)] transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${triggerClassName}`}
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`absolute left-0 right-0 top-full z-50 bg-[#080b2a]/95 backdrop-blur-2xl border border-blue-500/50 rounded-2xl p-1.5 shadow-[0_10px_40px_rgba(0,0,0,0.8)] max-h-60 overflow-y-auto custom-scrollbar ${menuClassName}`}
          >
            {options.map((option) => {
              ...
              return (
                <button
                  ...
                  className={`w-full px-3.5 py-2.5 rounded-xl text-xs font-mono font-semibold flex items-center justify-between transition-all cursor-pointer text-left ${
                    isSelected
                      ? "bg-blue-600/40 text-white border border-blue-400/60 shadow-[0_0_15px_rgba(59,130,246,0.3)]"
                      : "text-slate-200 hover:bg-blue-950/60 hover:text-white"
                  }`}
                >
                  <div className="flex items-center gap-2.5 truncate">
                    {option.icon && <span className="shrink-0 text-blue-300">{option.icon}</span>}
